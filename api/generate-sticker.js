...
export default async function handler(req, res) {
  ...
  try {
    ...
    // --- Pollinations BEST (FLUX) call ---
    const url =
  `https://image.pollinations.ai/prompt/${encodeURIComponent(cleanPrompt)}` +
  '?nologo=true&private=true&width=3840&height=3840&model=flux&enhance=true';
    ...
    return res.status(200).json({ success: true, image: base64, method: 'Pollinations-FLUX' });
  } catch (err) {
    console.error(err);
